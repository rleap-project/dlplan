...
TEST(DLPTests, MultiInstance) {
    ...
    auto instance_0 = std::make_shared<InstanceInfo>(vocabulary, 0);
    ...
    State state_0_0(instance_0, {atom_0_3, atom_0_4});

    // Instance with graph consisting of nodes A,B,C,D and edges A->B,B->C,C->D
    auto instance_1 = std::make_shared<InstanceInfo>(vocabulary, 1);
    ...
    State state_1_0(instance_1, {atom_1_3, atom_1_4});

    // Instance where all atoms of instance1 and instance2 are added
    auto instance = std::make_shared<InstanceInfo>(vocabulary, 2);
    ...
    State state_0(instance, {atom_6, atom_7});
