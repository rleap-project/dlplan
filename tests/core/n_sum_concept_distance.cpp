...
TEST(DLPTests, NumericalSumConceptDistance) {
    ...
    std::shared_ptr<const Numerical> numerical = factory.parse_numerical("n_sum_concept_distance(c_primitive(start,0),r_primitive(conn,0,1),c_primitive(end,0))");
    ...
    std::shared_ptr<const Numerical> numerical2 = factory.parse_numerical("n_sum_concept_distance(c_primitive(start2,0),r_primitive(conn,0,1),c_primitive(end2,0))");
    ...
    std::shared_ptr<const Numerical> numerical3 = factory.parse_numerical("n_sum_concept_distance(c_primitive(start3,0),r_primitive(conn,0,1),c_primitive(end2,0))");
