...
TEST(DLPTests, NumericalSumConceptDistance) {
    ...
    auto atom_5 = instance->add_atom("start", {"D"});  // distance 2: D -> E -> A
    auto atom_6 = instance->add_atom("start", {"C"});  // distance 1: C -> A
    auto atom_7 = instance->add_atom("end", {"A"});
    auto atom_8 = instance->add_atom("end", {"B"});
    auto atom_9 = instance->add_atom("start2", {"B"});  // distance INF
    ...
    auto numerical_0 = factory.parse_numerical("n_sum_concept_distance(c_primitive(start,0),r_primitive(conn,0,1),c_primitive(end,0))");
    ...
    auto numerical_1 = factory.parse_numerical("n_sum_concept_distance(c_primitive(start2,0),r_primitive(conn,0,1),c_primitive(end2,0))");
    EXPECT_EQ(numerical_1->evaluate(state_0), std::numeric_limits<int>::max());

    // SumConceptDistance with no sources is defined as 0
    auto numerical_2 = factory.parse_numerical("n_sum_concept_distance(c_primitive(start3,0),r_primitive(conn,0,1),c_primitive(end2,0))");
