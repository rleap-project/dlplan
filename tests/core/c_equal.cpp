...
TEST(DLPTests, ConceptEqual) {
    ...
    std::shared_ptr<const Concept> concept2 = factory.parse_concept("c_equal(r_primitive(role_2,0,1),r_primitive(role_1,0,1))");
    EXPECT_EQ(concept2->evaluate(state).to_sorted_vector(), Index_Vec({0, 2, 3}));
    EXPECT_EQ(concept2->evaluate(state, caches)->to_sorted_vector(), Index_Vec({0, 2, 3}));
    EXPECT_EQ(concept2->evaluate({state}, caches)->to_sorted_vector(), Index_Vec({0, 2, 3}));
}
