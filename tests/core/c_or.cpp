...
TEST(DLPTests, ConceptOr) {
    ...
    std::shared_ptr<const Concept> concept = factory.parse_concept("c_or(c_primitive(concept_1,0),c_primitive(concept_2,0))");
    EXPECT_EQ(concept->evaluate(state).to_sorted_vector(), Index_Vec({0, 1, 2}));
    EXPECT_EQ(concept->evaluate(state, caches)->to_sorted_vector(), Index_Vec({0, 1, 2}));
    EXPECT_EQ(concept->evaluate({state}, caches)->to_sorted_vector(), Index_Vec({0, 1, 2}));
}
