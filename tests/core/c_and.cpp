...
TEST(DLPTests, ConceptAnd) {
    ...
    auto concept_ = factory.parse_concept("c_and(c_primitive(concept_1,0),c_primitive(concept_2,0))");
    EXPECT_EQ(concept_->evaluate(state_0), create_concept_denotation(*instance, {"B"}));
}
