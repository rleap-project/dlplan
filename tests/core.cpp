...
TEST(DLPTests, InstanceCreation) {
    ...
    EXPECT_EQ(numerical.evaluate(state1, caches), 1);
    caches.clear();
    EXPECT_EQ(numerical.evaluate(state2, caches), 1);
    caches.clear();

    State state3(instance, {a2, a3});
    State state4(instance, {2, 3});
    EXPECT_EQ(state3, state4);
    EXPECT_EQ(numerical.evaluate(state3, caches), 0);
    caches.clear();
    EXPECT_EQ(numerical.evaluate(state4, caches), 0);
    caches.clear();
}
