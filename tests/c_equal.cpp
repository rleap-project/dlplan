...
TEST(DLPTests, ConceptEqual) {
    ...
    Concept concept1 = factory.parse_concept("c_equal(role_1(0,1),role_2(0,1))");
    EXPECT_EQ(concept1.evaluate(state), dlplan::core::ConceptDenotation({0, 2, 3}));

    Concept concept2 = factory.parse_concept("c_equal(role_2(0,1),role_1(0,1))");
    EXPECT_EQ(concept2.evaluate(state), dlplan::core::ConceptDenotation({0, 2, 3}));
}
