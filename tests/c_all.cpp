...
TEST(DLPTests, ConceptAll) {
    ...
    EXPECT_EQ(concept.evaluate(state), dlplan::core::ConceptDenotation({0, 2, 3}));
}
...
TEST(DLPTests, ConceptAll2) {
    ...
    EXPECT_EQ(concept.evaluate(state_1), dlplan::core::ConceptDenotation({0, 2, 4}));  // location_1 not included because spanner_1 not in C, location_2 not included because spanner_2 not in C

    EXPECT_EQ(concept.evaluate(state_2), dlplan::core::ConceptDenotation({0, 1, 2, 4}));  // location_1 now included because only bob there and bob in C.
}
