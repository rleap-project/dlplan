...
TEST(DLPLTests, TupleIndexGeneratorForEachArity1Test) {
    ...
    std::for_each(result_1.begin(), result_1.end(), [&](TupleIndex tuple_index){
        atom_tuple_indices_1.push_back(novelty_base.tuple_index_to_atom_tuple(tuple_index));
    });
    ...
    std::for_each(result_2.begin(), result_2.end(), [&](TupleIndex tuple_index){
        atom_tuple_indices_2.push_back(novelty_base.tuple_index_to_atom_tuple(tuple_index));
    });
    ...
    std::for_each(result_3.begin(), result_3.end(), [&](TupleIndex tuple_index){
        atom_tuple_indices_3.push_back(novelty_base.tuple_index_to_atom_tuple(tuple_index));
    });
...
TEST(DLPTests, TupleIndexGeneratorArity2Test) {
    ...
    auto tuple_index_generator = TupleIndexGenerator<2>(novelty_base, {}, {0,1});
    auto begin = tuple_index_generator.begin();
    auto end = tuple_index_generator.end();
    ...
    tuple_index_generator = TupleIndexGenerator<2>(novelty_base, {}, {1,0});
    ...
    tuple_index_generator = TupleIndexGenerator<2>(novelty_base, {}, {});
    begin = tuple_index_generator.begin();
    end = tuple_index_generator.end();
    EXPECT_EQ(*(begin++), 24);  // {4,4} 4*5^0 + 4*5^1
    EXPECT_EQ(begin, end);
    ...
    // Test for more than one atom tuple.
    tuple_index_generator = TupleIndexGenerator<2>(novelty_base, {}, {0,1,2});
    begin = tuple_index_generator.begin();
    end = tuple_index_generator.end();
    EXPECT_EQ(*(begin++), 5);  // {0,1} 0*5^0 + 1*5^1
