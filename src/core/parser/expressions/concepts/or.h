...
protected:
    virtual element::Concept_Ptr parse_concept_element_impl(ElementCache &cache) const override {
        if (m_children.size() != 2) {
            throw std::runtime_error("OrConcept::parse_concept_element - number of children ("s + std::to_string(m_children.size()) + " != 1).");
        }
        // 1. Parse children
        ...
        if (!(l && r)) {
            throw std::runtime_error("OrConcept::parse_concept_element - children are not of type Concept.");
        }
        // 2. Construct element
