...
public:
    ...
    std::shared_ptr<const dlplan::core::Concept> parse_concept(SyntacticElementFactory& factory) const override {
        if (m_children.size() != 2) {
            throw std::runtime_error("EqualConcept::parse_concept - number of children ("s + std::to_string(m_children.size()) + " != 2).");
        }
        // 1. Parse children
        auto role_left = m_children[0]->parse_role(factory);
        auto role_right = m_children[1]->parse_role(factory);
        if (!(role_left && role_right)) {
            throw std::runtime_error("EqualConcept::parse_concept - at least one children is a nullptr");
        }
        // 2. Construct element
