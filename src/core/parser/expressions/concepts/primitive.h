...
protected:
    virtual element::ConceptElement_Ptr make_concept_element_impl(const InstanceInfoImpl& info, ElementCache &cache) const override {
        ...
        if (!info.exists_predicate_name(predicate_name)) {
        ...
        unsigned predicate_idx = info.predicate_idx(predicate_name);
        ...
        return std::make_shared<element::PrimitiveConceptElement>(goal, predicate_idx, object_idx);
    }
