...
protected:
    virtual element::Numerical_Ptr parse_numerical_impl(const VocabularyInfoImpl& vocabulary, ElementCache &cache) const override {
        if (m_children.size() != 3) {
            throw std::runtime_error("ConceptDistance::parse_numerical_impl - number of children ("s + std::to_string(m_children.size()) + " != 3).");
        }
        // 1. Parse children
        ...
        if (!(concept_from && role && concept_to)) {
            throw std::runtime_error("ConceptDistance::parse_numerical_impl - child is not of type Concept, Role, Concept.");
        }
