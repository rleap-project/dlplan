...
protected:
    virtual element::NumericalElement_Ptr parse_numerical_element_impl(ElementCache &cache) const override {
        ...
        // 1. Parse children
        element::ConceptElement_Ptr concept_element = m_children[0]->parse_concept_element(cache);
        if (concept_element) {
            return std::make_shared<element::CountNumericalElement<element::ConceptElement_Ptr>>(concept_element);
        }
        element::RoleElement_Ptr role_element = m_children[0]->parse_role_element(cache);
        if (role_element) {
