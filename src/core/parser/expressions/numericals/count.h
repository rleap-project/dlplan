...
protected:
    virtual element::Numerical_Ptr parse_numerical_element_impl(ElementCache &cache) const override {
        ...
        if (concept_element) {
            return std::make_shared<element::CountNumerical<element::Concept_Ptr>>(concept_element);
        }
        element::Role_Ptr role_element = m_children[0]->parse_role_element(cache);
        if (role_element) {
            return std::make_shared<element::CountNumerical<element::Role_Ptr>>(role_element);
        }
        // 2. Construct element
