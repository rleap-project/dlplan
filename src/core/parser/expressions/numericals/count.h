...
protected:
    virtual element::NumericalElement_Ptr make_numerical_element_impl(std::shared_ptr<TaskInfo> task_info, ElementCache &cache) const override {
        ...
        element::ConceptElement_Ptr concept_element = m_children[0]->make_concept_element(task_info, cache);
        if (concept_element) {
            return std::make_shared<element::CountNumericalElement<element::ConceptElement_Ptr>>(task_info, concept_element->goal(), concept_element);
        }
        element::RoleElement_Ptr role_element = m_children[0]->make_role_element(task_info, cache);
        if (role_element) {
            return std::make_shared<element::CountNumericalElement<element::RoleElement_Ptr>>(task_info, role_element->goal(), role_element);
        }
