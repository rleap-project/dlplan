...
const ConceptDenotations* Concept::evaluate(const States& states, DenotationsCaches& caches) const {
    auto cached = caches.get_concept_denotations_cache().get_denotation(get_index());
    if (cached) return cached;
    ...
    caches.get_concept_denotations_cache().insert_denotation(get_index(), result_denotations);
