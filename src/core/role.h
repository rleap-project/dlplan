...
public:
    ...
    RoleDenotation evaluate(const Role* parent, const State& state, PerElementEvaluationCache& caches) const {
        ...
        if (state.get_instance_info() != caches.get_instance_info()) {
            throw std::runtime_error("RoleImpl::evaluate - mismatches instances of State and EvaluationCache.");
        }
