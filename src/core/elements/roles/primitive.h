    ...
    void compute_result(const State& state, RoleDenotation& result) const {
        ...
        for (int atom_idx : state.get_atom_idxs()) {
            ...
            if (atom.get_predicate_idx() == m_predicate.get_index()) {
                assert(dlplan::utils::in_bounds(m_pos_1, atom.get_object_idxs()));
                assert(dlplan::utils::in_bounds(m_pos_2, atom.get_object_idxs()));
                result.insert(std::make_pair(atom.get_object_idxs()[m_pos_1], atom.get_object_idxs()[m_pos_2]));
            }
        }
