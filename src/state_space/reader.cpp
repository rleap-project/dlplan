...
static void parse_atom(const std::string& atom_name, InstanceInfo& instance_info, bool is_static, bool is_goal, std::vector<int>& new_atom_indices) {
...
static std::vector<int> parse_atoms_file(const std::string& filename, InstanceInfo& instance_info) {
    ...
    while (infile >> name) {
        parse_atom(name, instance_info, false, false, new_atom_indices);
    }
