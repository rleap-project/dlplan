...
private:
    ...
    static std::function<ConceptTaskResult(const States&, const core::Role&, const core::Concept&, core::element::GeneratorEvaluationCaches&)> m_equal_concept_task;
            ...
            for (int i = 1; i <= target_complexity - 1; ++i) {
                ...
                for (const auto& r1 : data.m_roles_by_iteration[i]) {
                    for (const auto& r2 : data.m_roles_by_iteration[j]) {
                        m_tasks.push_back(th.submit(std::cref(m_equal_concept_task), std::cref(states), core::Role(r2), std::move(factory.make_equal_concept(r1, r2)), std::ref(caches)));
                    }
                }
            }
        }
