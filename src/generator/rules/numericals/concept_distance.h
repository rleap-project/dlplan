...
private:
    ...
    static std::function<NumericalTaskResult(const States&, const core::Concept&, const core::Role&, const core::Numerical&, core::element::GeneratorEvaluationCaches&)> m_concept_distance_task;
            ...
            for (const auto& c1 : data.m_concepts_by_iteration[i]) {
                for (const auto& r : data.m_roles_by_iteration[j]) {
                    for (const auto& c2 : data.m_concepts_by_iteration[k]) {
                        m_tasks.push_back(th.submit(std::cref(m_concept_distance_task), std::cref(states), core::Concept(c1), core::Role(r), std::move(factory.make_concept_distance(c1, r, c2)), std::ref(caches)));
                    }
                }
            }
        }
