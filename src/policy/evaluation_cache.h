...
public:
    ...
    bool get_feature_evaluation(const BooleanFeature& feature, const core::State& state) {
        ...
        if (!view.test(start)) {
            ...
            if (feature.get_boolean().evaluate(state))
                view.set(start + 1);
            view.set(start);
        }
...
public:
    ...
    int get_feature_evaluation(const NumericalFeature& feature, const core::State& state) {
        int index = feature.get_index();
        auto view = m_data[state];
        // -1 represents that the value is not cached.
        if (view[index] == -1) {
            view[index] = feature.get_numerical().evaluate(state);
        }
        return view[index];
    }
};
