...
bool EvaluationCache::retrieve_or_evaluate(const core::Boolean& boolean, const core::State& state) {
    if (state.get_index() < 0) {
        throw std::runtime_error("EvaluationCache::retrieve_or_evaluate: state_idx index cannot be negative.");
    }
    auto view = m_boolean_denots_cache[state.get_index()];
...
int EvaluationCache::retrieve_or_evaluate(const core::Numerical& numerical, const core::State& state) {
    if (state.get_index() < 0) {
        throw std::runtime_error("EvaluationCache::retrieve_or_evaluate: state_idx index cannot be negative.");
    }
    auto view = m_numerical_denots_cache[state.get_index()];
