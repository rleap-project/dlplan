...
int main() {
    ...
    // Evaluate the policy.
    assert(policy.evaluate_lazy(2, s2, 1, s1).second);
    assert(!policy.evaluate_lazy(2, s2, 0, s0).second);
    assert(!policy.evaluate_lazy(1, s1, 2, s2).second);
    assert(!policy.evaluate_lazy(0, s0, 2, s2).second);
