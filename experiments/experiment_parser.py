...
def get_num_generated_features_complexity_5(content, props):
    ...
    if props.get("time_complexity_5") is not None:
...
def get_num_generated_features_complexity_10(content, props):
    # All patterns are parsed before functions are called.
    if props.get("time_complexity_10") is not None:
...
parser = Parser()
parser.add_pattern("time_complexity_5", r"\[t=(.+)s, .+ KB\] Complexity 5:\n", type=float)
parser.add_pattern("memory_complexity_5", r"\[t=.+s, (.+) KB\] Complexity 5:\n", type=int)
parser.add_pattern("time_complexity_10", r"\[t=(.+)s, .+ KB\] Complexity 10:\n", type=float)
parser.add_pattern("memory_complexity_10", r"\[t=.+s, (.+) KB\] Complexity 10:\n", type=int)
